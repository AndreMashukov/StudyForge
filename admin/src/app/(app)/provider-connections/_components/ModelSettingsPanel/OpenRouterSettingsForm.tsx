...
} from '@admin/mutations/model-settings';
import {
...
export function OpenRouterSettingsForm({
  ...
  const handleSave = async (values: IOpenRouterSettingsFormValues) => {
    ...
    try {
      const { response, payload } = await saveOpenRouterSettings(
        normalizeOpenRouterSettingsSubmitPayload(values)
      );
  ...
  const handleTest = async () => {
    ...
    try {
      const { response, payload } = await testOpenRouterConnection();
      ...
      <CardContent className="space-y-5">
        ...
        <div className="grid gap-3 text-sm sm:grid-cols-3">
          ...
          <div className="rounded-lg border border-border p-4">
            ...
            <p className="mt-2">
              {formatModelsSyncedAt(openRouterConnection.modelsSyncedAt)}
            </p>
            <p className="mt-1 text-xs text-muted-foreground">
