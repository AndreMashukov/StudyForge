...
import { saveMiniMaxSettings, testMiniMaxConnection } from '@admin/mutations/model-settings';
import {
...
export function MiniMaxSettingsForm({
  ...
  const handleSave = async (values: IMiniMaxSettingsFormValues) => {
    ...
    try {
      const { response, payload } = await saveMiniMaxSettings(
        normalizeMiniMaxSettingsSubmitPayload(values)
      );
  ...
  const handleTest = async () => {
    ...
    try {
      const { response, payload } = await testMiniMaxConnection();
