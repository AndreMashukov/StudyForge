...
import { saveTogetherSettings, testTogetherConnection } from '@admin/mutations/model-settings';
import {
...
export function TogetherSettingsForm({
  ...
  const handleSave = async (values: ITogetherSettingsFormValues) => {
    ...
    try {
      const { response, payload } = await saveTogetherSettings(
        normalizeTogetherSettingsSubmitPayload(values)
      );
  ...
  const handleTest = async () => {
    ...
    try {
      const { response, payload } = await testTogetherConnection();
