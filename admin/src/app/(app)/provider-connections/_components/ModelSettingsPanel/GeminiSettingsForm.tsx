...
import { saveGeminiSettings, testGeminiConnection } from '@admin/mutations/model-settings';
import {
...
export function GeminiSettingsForm({
  ...
  const handleSave = async (values: IGeminiSettingsFormValues) => {
    ...
    try {
      const { response, payload } = await saveGeminiSettings(
        normalizeGeminiSettingsSubmitPayload(values)
      );
  ...
  const handleTest = async () => {
    ...
    try {
      const { response, payload } = await testGeminiConnection();
