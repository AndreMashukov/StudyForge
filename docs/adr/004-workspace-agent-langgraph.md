...
4. Stream real token deltas only for the final planner reply through `@study-forge/backend-llm` `onDelta`. Planning and executor steps emit status SSE events only. Do not use LangGraph `astreamEvents`.
