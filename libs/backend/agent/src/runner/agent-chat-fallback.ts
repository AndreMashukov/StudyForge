...
function summarizeListDirectories(result: unknown): string {
  ...
  const more =
    paths.length > preview.length ? ` (+${paths.length - preview.length} more)` : '';
...
function summarizeListRules(result: unknown): string {
  ...
  const names = result
    .map((entry) => (isRecord(entry) ? asString(entry.name) ?? asString(entry.id) : undefined))
    .filter((value): value is string => Boolean(value));
  if (names.length === 0) {
  ...
  const more =
    names.length > preview.length ? ` (+${names.length - preview.length} more)` : '';
...
function summarizeListDocuments(result: unknown): string {
  ...
  const titles = result
    .map((entry) => (isRecord(entry) ? asString(entry.title) ?? asString(entry.id) : undefined))
    .filter((value): value is string => Boolean(value));
  if (titles.length === 0) {
  ...
  const more =
    titles.length > preview.length ? ` (+${titles.length - preview.length} more)` : '';
